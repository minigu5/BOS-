...
※ 입력 신호(전처리)를 바꾼 경우 반드시 1_preprocess.py 를 먼저 다시 실행한 뒤
  이 스크립트로 재학습해야 한다. 기존 best_model.pth 는 옛 분포라 무효.
"""
...
DEFAULT_CONFIG = {
    ...
    "num_workers": 4,
    "seed": 42,
    "fp_penalty_weight": 1.0,  # 기본은 표준 BCE. 오탐 억제는 threshold 스윕으로 처리 (학습 퇴화 방지)
    "clf_threshold": 0.5,      # 이진 분류 임계값 (높일수록 오탐 감소)
    "patience": 3,             # Early Stopping 인내 에포크 (과적합 빠르므로 짧게)
    "dropout": 0.7,
}
...
class FPPenaltyBCELoss(nn.Module):
    ...
    def __init__(self, fp_weight: float = 2.0) -> None:  # 실효 기본값(2.0)과 일치
        super().__init__()
...
def compute_metrics(
    ...
    # mean_prob / pos_rate: 출력이 한쪽으로 쏠리는 퇴화(degenerate collapse) 조기 감지용
    return {
        "accuracy": acc, "f1": f1, "precision": prec, "recall": rec,
        "tp": int(tp), "tn": int(tn), "fp": int(fp), "fn": int(fn),
        "mean_prob": float(probs.mean()) if probs.size else 0.0,
        "pos_rate":  float((probs >= threshold).mean()) if probs.size else 0.0,
    }
...
def main() -> None:
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        logger.info(
            ...
            f"{va_m['mean_prob']:>8.4f}  {va_m['pos_rate']*100:>6.1f}%"
        )

        # Validation Loss 기준 최고 모델 저장 (F1은 퇴화 상태(F1=0)에서 흡수되어 갱신 불가 → loss 사용)
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info(f"최고 모델 로드 (에포크 {best_epoch}, Val Loss={best_val_loss:.4f})")
